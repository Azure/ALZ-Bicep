...
|   2   | Custom Policy Definitions              | Configures Custom Policy Definitions at the `organization management group`, including the preview CloudHealth health-model definitions used by default assignments.                                                                                                | Management Groups.                                                     | [infra-as-code/bicep/modules/policy/definitions](https://github.com/Azure/ALZ-Bicep/tree/main/infra-as-code/bicep/modules/policy/definitions)                                   |
...
|   8   | Built-In and Custom Policy Assignments | Creates policy assignments to provide governance at scale. The AzureCloud default set assigns preview CloudHealth health-model policies by default; remediation creates the target resource group and health model unless disabled by bool parameter or `parExcludedPolicyAssignments`.                                                                                                                  | Management Groups, Log Analytics Workspace & Custom Policy Definitions | [infra-as-code/bicep/modules/policy/assignments/alzDefaults](https://github.com/Azure/ALZ-Bicep/tree/main/infra-as-code/bicep/modules/policy/assignments/alzDefaults)           |
|  8.1  | Workload Specific Policy Assignments   | Creates workload specific policy assignments to provide governance at scale,                                                                                                | Management Groups, Custom Policy Definitions                           | [infra-as-code/bicep/modules/policy/assignments/workloadSpecific](https://github.com/Azure/ALZ-Bicep/tree/main/infra-as-code/bicep/modules/policy/assignments/workloadSpecific) |
...
> [!NOTE]
> Preview CloudHealth health-model policies are enabled by default in the AzureCloud ALZ defaults. Step 2 deploys their definitions, and step 8 assigns them at Platform and Landing Zones scopes. Existing subscriptions under those scopes become eligible for remediation after upgrade. The remediation identity receives Contributor plus Role Based Access Control Administrator, which can write and delete role assignments.

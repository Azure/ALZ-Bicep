...
function New-PolicySetDefinitionsBicepInputTxtFile {
    ...
    Get-ChildItem -Recurse -Path "$rootPath/$definitionsLongSetPath" -Filter "*.json" -Exclude "*.parameters.json" | ForEach-Object {
        $policyDef = Get-Content $_.FullName | ConvertFrom-Json -Depth 100
    ...
    $policyDefCount = Get-ChildItem -Recurse -Path "$rootPath/$definitionsLongSetPath" -Filter "*.json" -Exclude "*.parameters.json" | Measure-Object
    $policyDefCountString = $policyDefCount.Count
